...
- **`AI_REVIEW_SAMPLES` repo variable (opt-in, off by default):** default `1` (single
  deterministic pass — the low-noise default). Set to `2`–`5` to sample each model N
  times at spread temperatures and union the findings. **Measured caveat:** on our own
  planted-bug corpus (small, single-issue diffs) this gave **0 pp recall for ~2.8× the
  findings** — the extra passes just reproduce the same hits. It only helps on **large,
  multi-file PRs** where one pass under-samples the diff, so leave it at `1` for normal
  PRs and enable it only for big diffs. If you do enable it, pair with the
  `ai-review-strict` label so `consensus` (cross-sample agreement) filters the added noise.
