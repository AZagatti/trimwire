...
| # | Risk | Mitigation |
|---|---|---|
| 1 | **Linux WebKitGTK rendering drift / instability** (the strongest counter; Tauri's own team flags it) | Keep the UI deliberately plain — exactly trimwire's existing restrained dashboard style; no exotic CSS/WebRTC. Run **Playwright against WebKit**, not just Chromium. Most drift is cosmetic. |
