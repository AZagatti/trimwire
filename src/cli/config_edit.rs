...
pub fn config_edit() -> Result<()> {
    ...
    println!(
        "{} couldn't open an editor ($EDITOR/$VISUAL unset and no nano/vim/vi on PATH).",
        render::warn()
    );
    println!(
    ...
    println!(
        "  or set one, e.g. {}.",
        render::accent("EDITOR=code trimwire config")
    );
    Ok(())
}
...
pub fn config_show(json: bool) -> Result<()> {
    ...
    // These are TOML comment lines (kept byte-compatible with the format so
    // `trimwire config show > x.toml` still parses) — no glyph prefix before
    // the leading `#`; only inline values/commands get the accent.
    println!("# trimwire — resolved effective config");
    println!("# active profile: {}", render::accent(&profile));
    ...
    println!(
        "# Run {} to opt in (the collector URL is a built-in default \
         pointing at the live api.trimwire.dev; override [share] endpoint to self-host).",
        render::accent("`trimwire share enable`")
    );
    println!(
        "# {} does a dry-run and shows what would be sent until you enable.",
        render::accent("`trimwire share stats`")
    );
    println!();
