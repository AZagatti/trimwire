...
- **Rate the AI reviewer's findings (👍/👎) after verifying each one.** Every PR runs the in-repo AI-review bot, which posts **inline review comments** on the diff (each carries a hidden `<!-- ai-review-meta … -->` marker). Once you've **checked a finding against the actual code**, add a reaction to that comment: **👍 (`+1`)** if it's a real, useful catch — react even when you defer the fix to a filed follow-up; **👎 (`-1`)** if it's a false positive / noise / wrong. React on the bot's **ROOT finding comments only** (not threaded replies); never react before verifying, and never blanket-react. This is the **only maintainer signal** the weekly tracker (`scripts/ai_review_track.py` → the `ai-review-data` branch, workflow `ai-review-track.yml`) mines to compute per-persona precision — `+1`/heart/hooray/rocket = accepted, `-1`/confused = rejected (reject wins), none = `open`; a **wrong** label pollutes the tuning data worse than leaving it un-reacted. Applies to your own PRs too — react after the specialist-panel verify pass, before merge. Enumerate + react:
  `gh api repos/AZagatti/trimwire/pulls/<N>/comments --paginate --jq '.[]|select(.user.type=="Bot")|select(.body|test("ai-review-meta"))|{id,path,line}'`
  then `gh api -X POST repos/AZagatti/trimwire/pulls/comments/<comment_id>/reactions -f content='+1'` (or `-1`).
