...
| `trimwire stats [--json] [-q] [--since YYYY-MM-DD] [--until YYYY-MM-DD]` | Savings ledger: total saved, reduction %, ~tokens removed, per-strategy gauge (bars by bytes trimmed), cache-prefix stability. `--since`/`--until` restrict to a UTC date window ("what did I save this week?"); `--json` for scripting; `-q`/`--quiet` for a one-line headline. |
